// SimTools
// Main Application
// SimTools Sims 2 Gameplay Fixes Code-Behind
// (C) Archeon Industries, LLC. 2024 - 2026, All Rights Reserved.

// 'ERE'S THE BEEF! This is where the meat is. The bulkiest bit of fixes for The Sims 2, all in one place.
...
//  Window code-behind
// ══════════════════════════════════════════════════════════════════════════════
public partial class TS2GameplayFixesWindow : Window
{
    // ── State ─────────────────────────────────────────────────────────────────
    private readonly string _sims2Mods;
    private readonly List<GameplaySectionViewModel> _sections;

    // ── Constructor ───────────────────────────────────────────────────────────
    public TS2GameplayFixesWindow(string sims2Mods)
    {
        _sims2Mods = sims2Mods;
        InitializeComponent();
        ApplyLanguage();
        ...
        _sections = BuildSections()
            .Select(s =>
            {
                var vmItems = s.Items.Select(item =>
                {
                    ...
                    if (vm.IsActive && GamePaths.IsConfigured(_sims2Mods))
                    {
                        var destPath = Path.Combine(_sims2Mods, vm.FileName);
    ...
    {
        if (!GamePaths.IsConfigured(_sims2Mods))
        {
            WpfMessageBox.Show(
                LanguageManager.Get("Paths", "Sims2Mods",
        ...
        try
        {
            foreach (var item in toDownload)
            {
                ...
                var destPath = Path.Combine(_sims2Mods, item.FileName);
